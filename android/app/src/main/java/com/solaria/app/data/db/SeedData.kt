 ...
 *
 * When the backend (Express) or live data sources are connected,
 * they will write directly to these same tables in real-time.
 */
class SeedData(private val dbProvider: () -> AppDatabase) : RoomDatabase.Callback() {
    ...
    override fun onCreate(db: SupportSQLiteDatabase) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            seedPriceHistory(database)
        }
    }

    private suspend fun seedPriceHistory(db: AppDatabase) {
        val now = System.currentTimeMillis()
        val hour = 3_600_000L
        val day = 86_400_000L
        val week = 604_800_000L

        // Seed 24 hours of hourly SOL data
        val hourly = (0..23).map { i ->
            HourlyPriceEntity("solana", now - (23 - i) * hour, 140.0 + i * 0.5)
        }
        db.priceHistoryDao().insertHourlyBatch(hourly)

        // Seed 30 days of daily SOL data
        val daily = (0..29).map { i ->
            DailyPriceEntity("solana", now - (29 - i) * day, 120.0 + i * 1.0)
        }
        db.priceHistoryDao().insertDailyBatch(daily)

        // Seed 10 weeks of weekly SOL data
        val weekly = (0..9).map { i ->
            WeeklyPriceEntity("solana", now - (9 - i) * week, 100.0 + i * 5.0)
        }
        db.priceHistoryDao().insertWeeklyBatch(weekly)
    }
    ...
    private suspend fun seedPriceCache(db: AppDatabase) {
        ...
        // SOL with 7-day history (Realistic Price ~$145.00)
        val solHistory = (0..167).map { i ->
            """{"timestamp":${now - (167 - i) * hour},"price":${142.0 + Math.sin(i * 0.1) * 5 + i * 0.05}}"""
        }
        db.priceCacheDao().upsert(
            PriceCacheEntity(
                symbol = "SOL", price = 145.24, change24h = 4.15,
                volume24h = 3_240_000_000.0, marketCap = 65_200_000_000.0,
                historyJson = "[${solHistory.joinToString(",")}]"
            )
        )

        // BONK (Meme coin, real market data)
        db.priceCacheDao().upsert(
            PriceCacheEntity(
                symbol = "BONK", price = 0.000028, change24h = 12.4,
                volume24h = 240_000_000.0, marketCap = 1_860_000_000.0, historyJson = null
            )
        )
        // JUP (Jupiter, real market data)
        db.priceCacheDao().upsert(
            PriceCacheEntity(
                symbol = "JUP", price = 1.12, change24h = -1.5,
                volume24h = 110_000_000.0, marketCap = 1_540_000_000.0, historyJson = null
            )
        )
        // PYTH (Pyth Network, real market data)
        db.priceCacheDao().upsert(
            PriceCacheEntity(
                symbol = "PYTH", price = 0.45, change24h = 2.8,
                volume24h = 85_000_000.0, marketCap = 670_000_000.0, historyJson = null
            )
        )
    }
