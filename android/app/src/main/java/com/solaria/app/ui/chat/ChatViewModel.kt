...
class ChatViewModel @Inject constructor(
                ...
                val action = if (response.actionType == "TRANSFER" && response.actionAmount != null && response.actionTo != null) {
                    BotAction(
                        ...
                        payload = TransactionPayload(
                            unsignedTxBase64 = "pending_signing",
    ...
    // ── Approve a pending transaction (NOW REAL!) ─────────
        ...
        viewModelScope.launch {
            try {
                ...
                if (wallet != null && pending.payload != null) {
                    ...
                    if (hasKeypair) {
                        // REAL TRANSACTION: Build, sign, and broadcast on DevNet
                        repo.insertChatMessage(
                            ChatMessageEntity(
                                conversationId = conversationId, role = "bot",
                                text = "⏳ Building and signing transaction…"
                            )
                        )

                        val txSignature = repo.sendSol(
                        ...
                        repo.insertChatMessage(
                            ChatMessageEntity(
                                conversationId = conversationId, role = "bot",
                                text = "✅ Transaction confirmed on DevNet!\n\n" +
                                    "TX: ${txSignature.take(20)}…\n" +
                                    "Amount: ${pending.payload.amountSol} SOL\n" +
                                    "To: ${pending.payload.to.take(8)}…\n\n" +
                        ...
                        repo.insertChatMessage(
                            ChatMessageEntity(
                                conversationId = conversationId, role = "bot",
                                text = "📋 Transaction queued (watch-only wallet — no private key).\n" +
                                    "ID: ${tx.id.take(8)}…\n" +
                                    "Import your private key in Wallet settings to sign and broadcast."
                            )
                        )
                    }
                }
