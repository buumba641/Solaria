 ...
 *
 * Placeholder — cross-chain settlement on Solana is handled by
 * POST /api/settle-intent (payment.js) which invokes the Anchor
 * program's settle_payment instruction signed by the oracle.
 * This stub exists for API contract compatibility.
 */
router.post("/lifi/settle", requireApiKey, async (_req, res) => {
  res.json({ status: "mock", note: "Use POST /api/settle-intent for real on-chain settlement" });
});
